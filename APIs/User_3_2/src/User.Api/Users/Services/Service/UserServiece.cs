...
public class UserService : IUserService
{
    ...
    public Guid AddUser(UserRegisterDto userRegisterDto)
    {
        ...
        var newUser = new YouTubeUser
        {
            ...
            NickName = NickNameControl.NickNameUnique(userRegisterDto.UserName),
            CreatedAt = DateTime.UtcNow,
            UpdatedAt = DateTime.UtcNow,
            UserBlocked = false,
            Password = PasswordControl.SavePassword(userRegisterDto.Password, DateTime.UtcNow),
            Position = "User"
        };
        users.Add(newUser);
    ...
    public bool DeleteUserChannel(Guid channelId, string token)
    {
        ...
        var channelDefoult = ChannelRepository.GetAllChannels().Where(c => c.ChannelId == channelId).FirstOrDefault();
        if (channelDefoult == null)
        {
            return false;
        }

        var channels = ChannelRepository.GetAllChannels().Where(c => c.ChannelId != channelId).ToList();
        var channelVideos = VideoRepository.GetAllVideos().Where(v => v.ChannelId != channelId).ToList();

        ChannelRepository.SaveAllChannels(channels);
        VideoRepository.SaveAllVideos(channelVideos);
